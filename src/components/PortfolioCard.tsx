    ...
    <div ref={ref} onClick={() => onClick(item)}
      ...
      <div className="relative aspect-video overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-[#1a1a2e] via-[#0d0d1a] to-[#080812] flex items-center justify-center">
          <Play className="w-14 h-14 text-[#e8a838]/20" />
        </div>
        ...
        <span className="absolute top-3 left-3 px-3 py-1.5 text-[11px] font-semibold bg-black/40 backdrop-blur-sm text-white/80 rounded-full">{catLabels[item.category] || item.category}</span>
        <span className="absolute top-3 right-3 px-3 py-1.5 text-[11px] text-white/40 bg-black/40 backdrop-blur-sm rounded-full">{item.year}</span>
      </div>
      <div className="p-5">
        <h3 className="text-white font-bold text-lg mb-1.5 leading-tight group-hover:text-[#e8a838] transition-colors">{item.title}</h3>
        <p className="text-white/40 text-sm leading-relaxed line-clamp-2 mb-4">{item.description}</p>
        <div className="flex flex-wrap gap-2">
          {item.tags.map(t => <span key={t} className="px-3 py-1 text-xs bg-white/5 text-white/40 rounded-full">{t}</span>)}
        </div>
      </div>
    </div>
  )
