...
export default function VideoModal({ item, onClose }: { item: PortfolioItem; onClose: () => void }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center bg-[#0d0d1a]/95 backdrop-blur-2xl transition-all duration-500 ${vis ? 'opacity-100' : 'opacity-0'}`} onClick={handleBg}>
      <div className={`relative w-full max-w-5xl mx-4 transition-all duration-700 delay-200 ${vis ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="w-10 h-10 rounded-full glass-card flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all"><X className="w-4 h-4" /></button>
        </div>
        {item.videoType === 'direct' ? (
          ...
          <div className="aspect-video bg-gradient-to-br from-[#1a1a2e] to-[#0d0d1a] rounded-2xl flex items-center justify-center border border-white/5">
            <p className="text-white/40">视频加载中...</p>
          </div>
        )}
        <div className="flex flex-wrap gap-2 mt-4">
          {item.tags.map(t => <span key={t} className="px-3 py-1 text-sm text-white/50 glass-card rounded-full">{t}</span>)}
          <span className="px-3 py-1 text-sm text-white/50 glass-card rounded-full">{item.year}</span>
        </div>
      </div>
    </div>
  )
